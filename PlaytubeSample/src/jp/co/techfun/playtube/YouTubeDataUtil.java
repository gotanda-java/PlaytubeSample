...
public final class YouTubeDataUtil {
	...
	private List<YouTubeVideoItem> getSearchResult(String keyword, Integer pageNum) {
		...
		try {
			...
			for (int i = 0; i < jsonArray.length(); ++i) {
				...
				// サムネイル画像
				String thumbnailURL = obj.getJSONObject("thumbnail").getString("hqDefault");

				// 動画リスト１件を生成
				YouTubeVideoItem item = new YouTubeVideoItem(title, mpeg4spURL, thumbnailURL);
	...
	private String getQueryURL(String keyword, Integer pageNum) {
		...
		try {
			...
			Log.w(getClass().getSimpleName(), "getQueryURL：検索キーワードの変換に失敗しました。", e);
		}
		return sb.toString();
	}
}
